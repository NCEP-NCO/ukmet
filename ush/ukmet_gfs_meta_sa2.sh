...
then
    exit
elif [ ${cyc} -eq "00" ] && [ `echo $COMIN | awk -F/ '{print $9}' | awk -F. '{print $1}'` = "gfs" ]
then 
...
export HPCGFS=${COMINgfs}/gfs.${PDY}/${cyc}/atmos/gempak
...
do
    ...
    gfsfhr="F${gfsfhr}"
    grid2="${COMINecmwf}/ecmwf.${PDYm1}/gempak/ecmwf_glob_${PDYm1}12"
...
do
    ...
    gfsfhr="F${gfsfhr}"
    grid3="${COMIN}/${RUN}.${PDY}/gempak/ukmet_hr_${PDY}00f${ukmetfhr}"
...
# when running from the ukmet during the 00z cycle 
# need to set the COMOUT and DBN_ALERT_TYPE to gfs

if [ ${cyc} -eq "00" ]
then
    cpfs ${metaname} ${COMOUT}/${mdl}_${PDY}_${cyc}_${metatype}
    ${DBNROOT}/bin/dbn_alert MODEL UKMET_GFS_METAFILE $job \
